...
def ping(host, timeout=10):
    ...
    if system == 'windows':
        # En Windows: -n 1 (número de intentos), -w timeout_ms (timeout en ms)
        # Convertir segundos a milisegundos y asegurar mínimo 1
        timeout_ms = max(1, int(float(timeout) * 1000))
        cmd = ['ping', '-n', '1', '-w', str(timeout_ms), str(host)]
    else:
        # En Linux/macOS: -c 1 (número de paquetes), -W timeout (segundos)
    ...
    try:
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                patterns = [
                    r'time=([\d.,]+)\s*ms',          # Unix/Linux estándar
                    r'time[<>=]([\d.,]+)\s*ms',       # time<X>ms
                    r'tiempo[<>=]?\s*=\s*([\d.,]+)\s*ms',  # Windows en español
                    r'tiempo=([\d.,]+)\s*ms',         # Variante común español
                    r'([\d.,]+)\s*ms\s*$'            # Último número antes de ms
                ]
                ...
                if '100% packet loss' in output_lower or 'tiempo de espera agotado' in output_lower or 'request timed out' in output_lower:
                    print(f"[ping] Timeout o pérdida de paquetes al hacer ping a {host}")
                    return None, 'timeout'
                elif 'name or service not known' in output_lower or 'could not find host' in output_lower or 'no se pudo encontrar el host' in output_lower:
...
app.config['USE_SSE'] = False
try:
    # Intentar conexión a Redis para habilitar SSE solo si está disponible
    _redis_client = redis.Redis.from_url(app.config['REDIS_URL'], socket_connect_timeout=1, socket_timeout=1)
    if _redis_client.ping():
        app.config['USE_SSE'] = True
        app.register_blueprint(sse, url_prefix='/stream')
        print("[SSE] Redis disponible. SSE habilitado.")
except Exception as e:
    # Redis no disponible: continuar sin SSE
    app.config['USE_SSE'] = False
    print(f"[SSE] Redis no disponible ({e}). SSE deshabilitado. La app funcionará sin notificaciones en tiempo real.")
...
def ping_server(ip, result_queue, timeout=10):
    ...
    try:
        ...
        system = platform.system().lower()
        if system == 'windows':
            # Windows: -n 1 (intentos), -w timeout_ms
            timeout_ms = max(1, int(float(timeout) * 1000))
            cmd = ['ping', '-n', '1', '-w', str(timeout_ms), ip]
        else:
            # Linux/macOS: -c 1 (intentos), -W timeout_s
            cmd = ['ping', '-c', '1', '-W', str(int(float(timeout))), ip]
        ...
        try:
            ...
            # Normalizar salida para comprobaciones multi-idioma
            output_lower = output.lower()
            
            # Intentar extraer la latencia con varios patrones
            time_patterns = [
                r'time=([\d.,]+)\s*ms',
                r'time[<>=]([\d.,]+)\s*ms',
                r'tiempo[<>=]?\s*=\s*([\d.,]+)\s*ms',
            ...
            # Si no hay latencia, comprobar recibidos = 1 (Windows/Unix español/inglés)
            if ('1 received' in output_lower or '1 packets received' in output_lower or '1 recibidos' in output_lower or
                'received = 1' in output_lower or 'recibidos = 1' in output_lower):
                result_queue.put((True, {
...
def continuous_ping(ip, result_queue, stop_event):
    ...
    def send_single_ping():
        try:
            ...
            system = platform.system().lower()
            if system == 'windows':
                timeout_ms = max(1, int(float(PING_TIMEOUT) * 1000))
                cmd = ['ping', '-n', '1', '-w', str(timeout_ms), ip]
            else:
            ...
            if result.returncode == 0:
                ...
                if not (
                    '1 received' in output or '1 packets received' in output or '1 recibidos' in output or
                    'Received = 1' in output or 'recibidos = 1' in output
                ):
                    return {
                        'success': False,
                ...
                if '100% packet loss' in output or '100% de pérdida' in output or 'Request timed out' in output or 'tiempo de espera agotado' in output:
                ...
                elif 'Name or service not known' in output or 'Nombre o servicio no conocido' in output or 'could not find host' in output or 'no se pudo encontrar el host' in output:
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(f"[INIT] host={host} port={port} debug={debug}")
